...
export class DetalhesPacoteComponent implements OnInit {
  ...
  public inicializaPacote(pacote){
    ...
    this.hotel.telefone = hot.contact['phone'].cityCode + ' ' + hot.contact['phone'].number;
    this.quartos = this.hotel['accommodations'].items;
*/
    this.swiper.setIndex(0);
    this.imagens = [];

    for(var t = 0 ; t < this.pacote.images.items.length; t++){
      var img = this.pacote.images.items[t].link;
      var tituloFoto = this.pacote.images.items[0].title;
      this.imagens.push(
        {
          path: img
        })
